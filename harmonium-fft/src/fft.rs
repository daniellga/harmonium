...
macro_rules! impl_fft {
    ($(($t: ty, $f1: ident, $f2: ident, $f3: ident, $f4: ident, $f5: ident, $f6: ident, $f7: ident, $f8: ident, $f9: ident, $f10: ident)),* $(,)?) => {
        $(
            impl HFft<$t, Ix1> for HArray<Complex<$t>, Ix1> {
            ...
            impl HFft<$t, Ix2> for HArray<Complex<$t>, Ix2> {
            ...
            impl HFft<$t, IxDyn> for HArray<Complex<$t>, IxDyn> {
            ...
            impl HFft<$t, Ix1> for HArray<$t, Ix1> {
                fn fft(&self) -> HArray<Complex<$t>, Ix1> {
                    let length = self.len();
                    let (fft, mut ndarray) = $f1(length);
            ...
            impl HFft<$t, Ix2> for HArray<$t, Ix2> {
                fn fft(&self) -> HArray<Complex<$t>, Ix2> {
                    let nrows = self.0.nrows();
                    let ncols = self.0.ncols();
                    let (fft, mut ndarray) = $f2(nrows, ncols);
            ...
            impl HFft<$t, IxDyn> for HArray<$t, IxDyn> {
                fn fft(&self) -> HArray<Complex<$t>, IxDyn> {
                    let nrows = self.0.len_of(Axis(0));
                    let ncols = self.0.len_of(Axis(1));
                    let (fft, mut ndarray) = $f2(nrows, ncols);
            ...
            #[cached]
            fn $f6(length: usize) -> (Arc<dyn Fft<$t>>, ArcArray1<Complex<$t>>) {
                let mut planner = FftPlanner::<$t>::new();
                let ndarray = ArcArray1::zeros(length);
            ...
            #[cached]
            fn $f8(length: usize) -> Arc<dyn Fft<$t>> {
                let mut planner = FftPlanner::<$t>::new();
